...
iSlider = function () {
  ...
  iSlider.prototype._renderItem = function (el, i) {
    ...
    var insertImg = function () {
     /* html = item.height / item.width > self.ratio ?
      '<a href=" '+ item.url +' "><img src="' + item.content + '"></a>' :
      '<a href=" '+ item.url +' "><img src="' + item.content + '"></a>';*/
        html='<a href=" '+ item.url +' "><img loadsrc="' + item.content + '"></a>';
        el.innerHTML = html;
        utils.loadzoomImg($(el));
    };
  ...
  iSlider.prototype._renderHTML = function () {
    ...
    outer.style.cssText = 'width:' + this.width + 'px;margin:0;padding:0;list-style:none;';
...
window.iSlider = iSlider;
	/*function adapt(){
		var h = $('div[id*="focusPic"]').find('.islider-pic').eq(0).find('img').height();
        h=h||$('div[id*="focusPic"]').find('.islider-pic').eq(1).find('img').height();
        var w = $(window).width();
		$('div[id*="focusPic"]').find('ul, li').css({
			height : h + 'px'
		});
	}
	setInterval(adapt, 100);*/
}());
return iSlider;