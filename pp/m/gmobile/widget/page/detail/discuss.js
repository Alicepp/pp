...
var app={
    ...
    bindEvent:function(){
        ...
        $('.dis-haslogin .btn-primary').on('click',function(){
        ...
        $('.com-score .ping').on('click',function(){
            var score=$(this).attr('score');
            $(this).addClass('active').siblings().removeClass('active');
            that.showHomePage(null,null,score);
        });
        $('#agree li').on('click',function(){
            $(this).addClass('select').siblings().removeClass('select');
        });
        $('.want-com').on('click',function(){
            that.checkIfLogin();
            $('.mask-foot').show();
        })
        $('.mask-foot').on('click',function(){
            $(this).hide();
        })
        $('.dis-text').on('click',function(){
            return false
        })
    },
    getCommentsCount:function(){
        ...
        $.ajax({
            ...
            success:function(data){
                that.$commentsCount.text(data.data.commentsCount);
    ...
    noLoginCallback:function(){
        location.href='http://i.m.emao.com/login';
    },
    ...
    addComment:function($el,tag,replyId,replyName,replyUserUrl){
        ...
        val= $.trim(val);
        var len=val.length;
        var score=$("#agree .select").attr('score');
        if(len<=0 || len>300){
            $.alert('内容限制1-300个字');
            return;
        }
...
*/
        ...
        if(tag=='home'){
            this.$el.prepend(that.getCommentTpl(dataList))
        }else{
